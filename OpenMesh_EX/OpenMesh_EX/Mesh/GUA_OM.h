...
public:
	Tri_Mesh()
	{
		add_property(copy_point_id);
		add_property(point_num);
		add_property(point_num_2d);
		add_property(weight_id);
		add_property(point_2d_id);
		add_property(weight);
		add_property(bind_2d_to_3d);
	...
	OMT::Point nearestpoint;
	OMT::Point temppoint;
	OMT::Point pointA;
	OMT::Point pointB;
	OMT::Point pointC;
	OMT::Point final_pointA;
	OMT::Point final_pointB;
	OMT::Point final_pointC;
	
	int facecount = 0;
	int pointcount = 0;
	int pickfacecount = 0;
	int pickroundfacecount = 0;
	int pick_face_firsttimecount = 0;
	int firsttimecount = 0;
	FVIter	tempfv_it;
	std::vector<OMT::Point> facepoint;
	std::vector<OMT::FaceHandle> pickface_h;
	std::vector<OMT::VertexHandle> pickface_vh;
	std::vector<OMT::FaceHandle> pickroundface_h;
	OMT::VPropHandleT<int> point_num;
	OMT::VPropHandleT<int> point_num_2d;
	OMT::VPropHandleT<double> weight_id;
	OMT::VPropHandleT<int> copy_point_id;
	OMT::VPropHandleT<int> point_2d_id;
	OMT::VPropHandleT<double> weight;
	OMT::VPropHandleT<OMT::Point> bind_2d_to_3d;
	int loadcount = 0;
	GLuint _textureID;

	std::vector<int> faceid;
	OMT::VertexHandle middlepoint_v_h;
	void Render_Solid();
	void Render_SolidWireframe();
	void Render_Wireframe();
	void Render_Point();
	OMT::VertexHandle findpoint(float x, float y, float z);
	OMT::FaceHandle findface(double x, double y, double z);
	std::vector<OMT::FaceHandle> mouse_pick_roundface(double x, double y, double z);
	std::vector<OMT::FaceHandle> mouse_pick_face(double x, double y, double z);
	std::vector<OMT::VertexHandle> mouse_pick_face_vh(double x, double y, double z);
	std::vector<double> turn_2d_point(OMT::VertexHandle);
	OMT::VertexHandle return_middle(double x, double y, double z);
	std::vector<OMT::VertexHandle> copy_point_repeat(std::vector<OMT::VertexHandle> copyvh, OMT::FaceHandle newfh);
	OMT::Point compute_middle_2d_vertex(std::vector<OMT::VertexHandle> vh, OMT::VertexHandle mid_vh);
	void clear_vh();
	void Render_Texture(std::string filepath);
	//std::vector<OMT::Point>
